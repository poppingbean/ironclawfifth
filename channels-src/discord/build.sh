...
if [ -f "$WASM_PATH" ]; then
    ...
    wasm-tools component new "$WASM_PATH" -o discord.wasm 2>/dev/null || { cp "$WASM_PATH" discord.wasm 2>/dev/null || powershell -Command "Copy-Item -Path '$WASM_PATH' -Destination 'discord.wasm'"; }
