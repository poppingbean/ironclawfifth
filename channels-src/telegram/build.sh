...
if [ -f "$WASM_PATH" ]; then
    ...
    wasm-tools component new "$WASM_PATH" -o telegram.wasm 2>/dev/null || { cp "$WASM_PATH" telegram.wasm 2>/dev/null || powershell -Command "Copy-Item -Path '$WASM_PATH' -Destination 'telegram.wasm'"; }
